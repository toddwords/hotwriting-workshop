...
window.onload = function() {
    ...
    //I'll be real here, this is some code I copy and pasted from the internet. It is a 'regular expression', which are an advanced way of manipulating strings of text. Basically this line of code takes all the keys (words to the left of the colon) in 'replacements' and puts them in one easy-to-search string
    var re = new RegExp("\\b(" + Object.keys(replacements).join("|") + ")\\b", "g");
    

    //This update function checks if the user has begun typing, then checks the textarea for any of the keys in replacements, then replaces those keys with the corresponding value, while also playing a typewriter sound
    update = function() {
        if(writingLetter){
    ...
    ta.onkeydown = function(e) {
        ...
        //this is the bread and butter of the piece, it runs everytime a key is pressed after the initial space
            ...
            if(writingLetter){
                //Whenever the user presses a key, instead of typing the corresponding letter of that key, we want to add the next letter of our poem text ('str') from the top. This way we can make sure we hit all of our fun text replacements in 'replacements'
